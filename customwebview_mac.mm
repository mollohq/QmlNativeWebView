...
CustomWebView::CustomWebView(QWindow *parent)
    : QWindow(parent), m_isInitialized(false), m_childWindow(nullptr), _wkWebView(nil)
{
...
void CustomWebView::initialize()
{
    if (!m_isInitialized) {
        try {
            ...
            _wkWebView = webView;
            _wkWebView.navigationDelegate = [[CustomWebViewDelegate alloc] initWithWebView:this];
